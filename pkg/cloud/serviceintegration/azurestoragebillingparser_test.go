...
func TestAzureStorageBillingParser_parseCSV(t *testing.T) {
			...
			expected: []AzureBillingValues{
				{
					...
					BillingID:      "11111111-12ab-34dc-56ef-123456billing",
			...
			expected: []AzureBillingValues{
				{
					...
					MeterCategory:  "Virtual Machines",
					SubscriptionID: "11111111-12ab-34dc-56ef-123456abcdef",
					BillingID:      "11111111-12ab-34dc-56ef-123456abcdef",
					InstanceID:     "/subscriptions/11111111-12ab-34dc-56ef-123456abcdef/resourceGroups/Example-Resource-Group/providers/Microsoft.Compute/virtualMachineScaleSets/aks-nodepool1-12345678-vmss",
					Service:        "Microsoft.Compute",
					Tags: map[string]string{
						"resourceNameSuffix": "12345678",
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			var actual []*AzureBillingValues
			resultFn := func(abv *AzureBillingValues) error {
			...
			if err != nil {
				t.Errorf("Error generating AzureBillingValues: %s", err.Error())
			}
			...
			for i, this := range actual {
				...
				if this.SubscriptionID != that.SubscriptionID {
					t.Errorf("Parsed data at index %d has incorrect MeterCategroy value. Expected: %s, Actual: %s", i, this.SubscriptionID, that.SubscriptionID)
				}

				if this.BillingID != that.BillingID {
					t.Errorf("Parsed data at index %d has incorrect MeterCategroy value. Expected: %s, Actual: %s", i, this.BillingID, that.BillingID)
				}

				if this.InstanceID != that.InstanceID {
					t.Errorf("Parsed data at index %d has incorrect MeterCategroy value. Expected: %s, Actual: %s", i, this.InstanceID, that.InstanceID)
				}

				if this.Service != that.Service {
					t.Errorf("Parsed data at index %d has incorrect MeterCategroy value. Expected: %s, Actual: %s", i, this.Service, that.Service)
				}

				if this.Cost != that.Cost {
					t.Errorf("Parsed data at index %d has incorrect MeterCategroy value. Expected: %f, Actual: %f", i, this.Cost, that.Cost)
				}

				if this.NetCost != that.NetCost {
					t.Errorf("Parsed data at index %d has incorrect MeterCategroy value. Expected: %f, Actual: %f", i, this.NetCost, that.NetCost)
				}
