...
// AzureBillingValues holder for Azure Billing Values
type AzureBillingValues struct {
	Date           time.Time
...
func (abv *AzureBillingValues) IsCompute(category string) bool {
	if category == kubecost.ComputeCategory {
		return true
	}

	if category == kubecost.StorageCategory || category == kubecost.NetworkCategory {
		if abv.Service == "Microsoft.Compute" {
			return true
		}
	}
	if category == kubecost.NetworkCategory && abv.MeterCategory == "Virtual Network" {
		return true
	}
	return false
}

// AzureBillingParser holds indexes of relevent fields in Azure Billing CSV in addition to the correct data format
type AzureBillingParser struct {
	Date           int
	MeterCategory  int
	BillingID      int
	SubscriptionID int
...
func NewAzureBillingParser(headers []string) (*AzureBillingParser, error) {
	...
	if i, ok := headerIndexes["usagedatetime"]; ok {
		...
		return nil, fmt.Errorf("NewAzureBillingParser: failed to find Date field")
	}
	...
	if i, ok := headerIndexes["subscriptionid"]; ok {
		...
		return nil, fmt.Errorf("NewAzureBillingParser: failed to find Subscription ID field")
	}
	...
	if i, ok := headerIndexes["billingaccountid"]; ok {
		...
		abp.BillingID = abp.SubscriptionID
	}
	...
	if i, ok := headerIndexes["instanceid"]; ok {
		...
		return nil, fmt.Errorf("NewAzureBillingParser: failed to find Instance ID field")
	}
	...
	if i, ok := headerIndexes["metercategory"]; ok {
		...
		return nil, fmt.Errorf("NewAzureBillingParser: failed to find Meter Category field")
	}
	...
	if i, ok := headerIndexes["tags"]; ok {
		...
		return nil, fmt.Errorf("NewAzureBillingParser: failed to find Tags field")
	}
	...
	if i, ok := headerIndexes["additionalinfo"]; ok {
		...
		return nil, fmt.Errorf("NewAzureBillingParser: failed to find Additional Info field")
	}
	...
	if i, ok := headerIndexes["consumedservice"]; ok {
		...
		return nil, fmt.Errorf("NewAzureBillingParser: failed to find Service field")
	}
	...
	if i, ok := headerIndexes["costinbillingcurrency"]; ok {
		...
		return nil, fmt.Errorf("NewAzureBillingParser: failed to find Net Cost field")
	}
...
func (abp *AzureBillingParser) ParseLine(start, end time.Time, record []string) *AzureBillingValues {
	usageDate, err := time.Parse(abp.DateFormat, record[abp.Date])
	if err != nil {
		// try other format, and switch if successful
		if abp.DateFormat == azureDateLayout {
			abp.DateFormat = AzureEnterpriseDateLayout
		} else {
			abp.DateFormat = azureDateLayout
		}
		...
		if err != nil {
			log.Errorf("failed to parse usage date: '%s'", record[abp.Date])
			return nil
		}
	}
	...
	cost, err := strconv.ParseFloat(record[abp.Cost], 64)
	if err != nil {
		log.Errorf("failed to parse cost: '%s'", record[abp.Cost])
		return nil
	}

	netCost, err := strconv.ParseFloat(record[abp.NetCost], 64)
	if err != nil {
		log.Errorf("failed to parse net cost: '%s'", record[abp.NetCost])
		return nil
	}

	additionalInfo := make(map[string]any)
	additionalInfoJson := encloseInBrackets(record[abp.AdditionalInfo])
	...
	tagJson := encloseInBrackets(record[abp.Tags])
	if tagJson != "" {
	...
	return &AzureBillingValues{
		Date:           usageDate,
		MeterCategory:  record[abp.MeterCategory],
		SubscriptionID: record[abp.SubscriptionID],
		BillingID:      record[abp.BillingID],
		InstanceID:     record[abp.InstanceID],
		Service:        record[abp.Service],
		Tags:           tags,
...
func AzureSetProviderID(abv *AzureBillingValues) string {
